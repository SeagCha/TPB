...
token = "5775920018:AAGGP8aA7A0kkae3SsMT9_eqfa2PvyaIeLQ"
...
def get_text_messages(message):
    ...
    if random.randint(0, 30) == 0 and bot.flag:
        print(bot.mass)
...
mas = ["бля", "пизда", "пиздец", "хуй", "похуй", "нахуй", "жопа", "ебло", "еблан", "хуесос", "уебок"]


@bot.message_handler(content_types=["text"])
def func(message):
    if message.text == "Что ты можешь?":
        bot.send_message(message.chat.id, text="Кидай голосовое сообщение и смотри сам, уебок тупорылый")

    if message.text == "Для чего тебя создали?":
        bot.send_message(message.chat.id, text="Я ебу?")

    for a in mas:
        if a in message.text.lower() and message.chat.id == -1001680613693:
            bot.send_message(message.chat.id, text="не матерись чурка")

    if "кто гей" in message.text.lower():
        x = "@abgshfk"
        bot.send_message(message.chat.id, text="гей вот это чмо - {}".format(x))
